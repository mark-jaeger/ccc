...
func TestBuildCreateCommand(t *testing.T) {
	...
	expected := "tmux new-session -d -s 'rt1' -c '/home/user/proj' \\; set-option -t 'rt1' @ccc_project 'rt1' \\; set-option -t 'rt1' @ccc_path '/home/user/proj' \\; set-option -t 'rt1' bell-action any \\; set-window-option -t 'rt1' visual-bell off \\; set-window-option -t 'rt1' allow-passthrough on"
	if cmd != expected {
		t.Errorf("expected:\n  %s\ngot:\n  %s", expected, cmd)
	}
}
